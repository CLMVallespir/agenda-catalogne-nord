...
| **4** | Els 7 ajudants de prova (punts 38–44 = els set del §5), i la **decisió sobre `CAMPS_CONTRASTABLES`** (§5, «Un cas a part»). | **Les 17 bateries offline passen** (16 el 5 de setembre, més `verifica-camps`). Sortida de cadascuna, mostrada. — **FETA el 16 de setembre de 2026**, i **són 6 llocs, no 7**: el setè, `fusionaFiles()`, ja va quedar tancat a la sessió 3 pel punt 16, que comparteix la mateixa constant `CAMPS`. `verifica-camps.js` torna **0** (26 comprovacions, 16 fitxers) i `verifica-enum.js` també. **`periodicitat` queda FORA de `CAMPS_CONTRASTABLES`**, amb el perquè escrit al comentari de `eines/verifica-esdeveniment.js`: `fitxaPerAlModel()` ensenya al model tots els contrastables buits inclosos —i la Q1 fa que arribi buit per un motiu nostre—, i `textPerVerificar()` donaria per verificable una fila que només tingués aquest camp, gastant una crida a Gemini per no contrastar res. **16 de les 17 bateries passen; `pipeline-offline` falla exactament igual que abans d'aquesta sessió** —mateix fitxer, mateixa línia 552, mateix `TypeError`, comprovat contra el repositori intacte—, i és el trencament que `NOTES.md` ja té anotat des de la sessió 3. La reutilització possible del `filaDeProva()` entre els dos mòduls de dedup queda anotada a `NOTES.md` i **no feta**. |
